...
- Blender 4.5.10 LTS Linux x64: https://download.blender.org/release/Blender4.5/blender-4.5.10-linux-x64.tar.xz
  - Checksum-verified portable authoring/export runtime for the editable Rotunda, entrance-pavilion, bowling-club, Kevin Murray Stand, Emely Baker Centre, Alfred Crescent Pavilion and north public-toilet assets; it is tooling, not physical-condition evidence.
...
- The 2026-07-10 audit removes the remaining approximate picnic tables because no public per-table points were found, removes a duplicate oval-loop path, keeps open sports fence runs from closing through clubhouses, and now records 578 fixed physical entries plus 53 explicit public-data gaps in `docs/research/edinburgh-gardens-2026-object-audit-ledger.json`. The Alfred pass adds a researched kiosk access component and resolves that building's former facade gap; the north-toilet pass adds two evidence-linked stall-bank interactions. Semantic toilet interaction halos remain outside the fixed-object count.
